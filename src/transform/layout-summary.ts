...
export function extractSchemaCanvasDimensions(
  schema: UnknownRecord,
): CanvasDimensions | undefined {
  const props = isRecord(schema.props) ? schema.props : {};
  const directStyle = isRecord(schema.style) ? schema.style : {};
  const propsStyle = isRecord(props.style) ? props.style : {};
  const style = { ...directStyle, ...propsStyle };
  const width = readDimension(style.width);
  const height = readDimension(style.height);
